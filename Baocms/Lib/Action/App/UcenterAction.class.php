...
class UcenterAction extends CommonAction {
    ...
    public function tx(){
        ...
        $user_info=$users->getUserByUid($user_id);
    ...
    public function zhifubao(){
        ...
        $users=D("Users");
        $status=$users->setzyb($user_id,$user_zfb,$user_name);

        if ($status){
            $this->ajaxReturn($user_zfb,"绑定成功");
        }else{
            $this->ajaxReturn($user_zfb,"绑定失败",0);
        }
    }
    ...
    public function pay_pwd(){
        $user_id=$this->uid;
        $zfb_pwd=$_POST['zfb_pwd'];
        $yzm=(int)$_POST['yzm'];
        //判断验证码
        $users= D("Users");
        $userinfo=$users->getUserByUid($this->uid);
        $mobile=$userinfo['account'];
        $num=Cac()->get('zhifu_code_'.$mobile);

        if ($yzm !=$num){
            $this->ajaxReturn(null,'验证码错误!');
        }
            Cac()->delete('zhifu_code_'.$mobile);
    ...
    public function get_renking(){
        ...
        $this->ajaxReturn(serialize($data),"排行榜");
    }
    ...
    public function haibao(){
        ...
        $value= $url = "https://www.darkhorse.vip/app/login/reg?pid=".$ID;					//二维码内容
    ...
    public function sendcode_zhifu(){
        //
        $userinfo=D("Users")->getUserByUid($this->uid);
        $mobile=$userinfo['account'];
        $code=rand_string(6,1);
        Cac()->set('zhifu_code_'.$mobile,$code,300);
        //todo 发送短信
        //Sms:LoginCodeSend($mobile,$code);
        $res=D("Sms")->dxbsend($mobile,$code);
    ...
    public function rechargelist(){
        ...
        foreach ($list as &$value){
            $value['creatime']=date('Y-m-d',$value['creatime']);
        }
