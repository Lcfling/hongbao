...
class ZhifuAction extends CommonAction{
    ...
    public function indexs(){
        ...
        //链接
        $URL= 'http://www.fczfpay.com/Api/CreateOrder';
        ...
        //订单金额
        $config=$this->config();
        $arrData['version'] = $config['version'];
        $arrData['customer_id'] = $config['customer_id'];
        $arrData['customer_order_no'] = $customer_order_no;
        $arrData['money'] = $money;
        $arrData['pay_type'] = $config['pay_type'];
        $arrData['notify_url'] = $config['notify_url'];
        $arrData['return_url'] = $config['return_url'];
        $arrData['return_type'] = $config['return_type'];


        //md5签名串
        $sign = $this->getSign($arrData, $config['key']);
        $arrData['sign'] = $sign;
        $re=$this->_request($arrData,$URL);
